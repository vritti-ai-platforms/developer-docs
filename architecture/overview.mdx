...
Vritti is a multi-tenant SaaS platform built with a micro-frontend architecture on the frontend and modular NestJS backends. The system prioritizes security, scalability, and developer experience.
...
flowchart TB
    ...
    subgraph APIs["API Layer"]
        APINexus["api-nexus<br/>(NestJS + Fastify + Drizzle ORM)"]
        CloudServer["cloud-server<br/>(NestJS + Fastify + Drizzle ORM)"]
    end
...
|---------|---------|------------|
| **web-nexus** | Host application for Module Federation | React 19, Rsbuild, TanStack Query |
| **cloud-web** | Standalone auth + account web app (remote) | React 19, Rsbuild, react-hook-form |
| **cloud-microfrontend** | Cloud dashboard micro-frontend (remote) | React 19, Rsbuild, TanStack Query |
...
| **@vritti/api-sdk** | NestJS modules for auth, database, logging | 0.2.x |

### Backend Services

| Service | Purpose | Stack |
|---------|---------|-------|
| **api-nexus** | Platform API gateway (auth, tenants, users, onboarding) | NestJS 11, Fastify 5, Drizzle ORM |
| **cloud-server** | Cloud product API (companies, deployments, AI chat) | NestJS 11, Fastify 5, Drizzle ORM |
