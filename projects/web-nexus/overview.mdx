...
web-nexus is the main host application in Vritti's micro-frontend architecture. It orchestrates and dynamically loads remote micro-frontends (cloud-web, cloud-microfrontend) using Module Federation with runtime registration.
...
flowchart TB
    ...
    subgraph Remotes["Remote Micro-frontends"]
        CloudWeb[cloud-web :3001]
        CloudMF[cloud-microfrontend :3002]
    end
    ...
    Router --> RemoteLoader
    RemoteLoader --> CloudWeb
    RemoteLoader --> CloudMF
    Providers --> Shared
    CloudWeb --> Shared
    CloudMF --> Shared
```
...
|----------|------|---------|---------|
| `PUBLIC_VRITTI_AUTH_PORT` | string | cloud-web remote port (local dev) | `3001` |
| `PUBLIC_VRITTI_CLOUD_PORT` | string | cloud-microfrontend port (local dev) | `3002` |
...
Unlike build-time module federation, web-nexus uses runtime registration:
...
if (isCloud) {
  routes.push({
    path: '/*',
    element: <RemoteRoutes remoteName="CloudWeb" moduleName="routes" />,
  });
}
```
...
**Production example**: With `PUBLIC_MF_BASE_URL=https://mf.vrittiai.com` → `https://mf.vrittiai.com/cloud-web/mf-manifest.json`
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="cloud-web" icon="user-lock" href="/projects/cloud-web/overview">
    Auth + account micro-frontend
  </Card>
  <Card title="cloud-microfrontend" icon="chart-bar" href="/projects/cloud-microfrontend/overview">
    Cloud dashboard micro-frontend
  </Card>
</CardGroup>
