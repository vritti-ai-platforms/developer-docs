...
<Accordion title="403 Forbidden: CSRF validation failed">
  ...
  **Solution:**
  ...
  - Check if `_csrf` cookie exists in browser DevTools
  - Call `/csrf/token` to get a fresh token
